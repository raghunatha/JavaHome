...
public class SpringHibernateUserDAO extends HibernateDaoSupport{
	...
	public User getUserByUNP(String username,String password){
		...
		return (User) getHibernateTemplate().findByCriteria(DetachedCriteria.forClass(User.class).add(Restrictions.eq("username", username)).add(Restrictions.eq("password", password))).get(0);
	}
